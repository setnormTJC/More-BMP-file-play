...
const lockFilePath = path.join(fileDirectory, 'lockfile'); 
...
app.post('/save-coordinates', (req, res) =>
{
    ...
    fs.writeFileSync(lockFilePath, ''); //creates lock file 

    const coordinatesText = coordinates.map(coord => `Mouse click at (${coord.x}, ${coord.y})\n`).join('');

    fs.appendFile(filePath, coordinatesText, (err) =>
    {
        if (err)
        {
